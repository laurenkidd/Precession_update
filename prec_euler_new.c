...
double functionx(double Ex, double vex, double Ey, double vey, double Ez, double vez, double Sx, double Sy, double Sz, double phi, double theta, double t){
	...
	I1     = 8.008e37   ;        //[kg m^2]
	I3     = 8.034e37 ;	    //[kg m^2]*/
	double d = sqrt(pow(Sx,2)+pow(Sy,2)+pow(Sz,2)) - sqrt(pow(Ex,2)+pow(Ey,2)+pow(Ez,2));
	double dx, dy, dz, d3;
	dx = Sx - Ex;
	dy = Sy - Ey;
	dz = Sz - Ez;
...
int main() {
	...
	/* xxxxxxxxxxxxxxxxxxxinitial conditionsxxxxxxxxxxxxxxxxxx
	//vec(Re(0)) = {r0, 0, 0} ; vec(ve(0)) = {0,v0,0}
	double r0, KE, v0,  psi0, phi0, vtheta0, vphi0, vpsi0;
	double Ex0, Ey0, Ez0, vex0, vey0, vez0;
	...
	//initial conditions for sun
	double Sx0, Sy0, Sz0, vsx0, vsy0, vsz0;
	Sx0 = 0; //let sun start at origin
	Sy0 = 0;
	Sz0 = 0;
	vsx0 = 0;
	vsy0 = -(me/ms)*v0;
	...
	double t=0,h=10000; 			//time, step size(sec).
...
/*double Ex[], double Ex0, double vex[],double vex0,double Ey[], double Ey0, double vey[],double vey0,  double Ez[], double Ez0, double vez[], double vez0, double Sx[], double Sx0,
double vsx[],  double Sy[], double Sy0, double vsy[], double Sz[], double Sz0, double vsz[], double phi[], double phi0, double vphi[], double vphi0, double theta[], double theta0, double vtheta[], double vtheta0, double psi[], double t, double h, long steps);*/
...
double vsx[],  double Sy[], double vsy[], double Sz[], double vsz[], double phi[], double vphi[],  double theta[], double vtheta[],  double psi[], double t, double h, long steps){
	...
	for ( i=1; i<steps; ++i){
		...
		k4x = funcx(Ex[i-1]+h*vex[i-1]+h*h*k3x/2.,vex[i-1]+h*k3x, Ey[i-1]+h*vey[i-1]+h*h*k3y/2.,vey[i-1]+h*k3y, Ez[i-1]+h*vez[i-1]+h*h*k3z/2.,vez[i-1]+h*k3z,
			Sx[i-1]+h*vsx[i-1]+h*h*k3sx/2., Sy[i-1]+h*vsy[i-1]+h*h*k3y/2., Sz[i-1]+h*vsz[i-1]+h*h*k3z/2., phi[i-1]+h*vphi[i-1]+h*h*k3ph/2., 
		...
		k4z = funcz(Ex[i-1]+h*vex[i-1]+h*h*k3x/2.,vex[i-1]+h*k3x, Ey[i-1]+h*vey[i-1]+h*h*k3y/2.,vey[i-1]+h*k3y, Ez[i-1]+h*vez[i-1]+h*h*k3z/2.,vez[i-1]+h*k3z,
			...
			theta[i-1]+h*vtheta[i-1]+h*h*k3t/2., t+h);
		k4sz = -k4z*me/ms;
		k4t = funct(Ex[i-1]+h*vex[i-1]+h*h*k3x/2., Ey[i-1]+h*vey[i-1]+h*h*k3y/2., Ez[i-1]+h*vez[i-1]+h*h*k3z/2.,Sx[i-1]+h*vsx[i-1]+h*h*k3sx/2., Sy[i-1]+h*vsy[i-1]+h*h*k3y/2., 			Sz[i-1]+h*vsz[i-1]+h*h*k3z/2., theta[i-1]+h*vtheta[i-1]+h*h*k3t/2.,vtheta[i-1]+h*k3t, phi[i-1]+h*vphi[i-1]+h*h*k3ph/2., vphi[i-1]+h*k3ph);
		k4ph = funcph(Ex[i-1]+h*vex[i-1]+h*h*k3x/2., Ey[i-1]+h*vey[i-1]+h*h*k3y/2., Ez[i-1]+h*vez[i-1]+h*h*k3z/2.,Sx[i-1]+h*vsx[i-1]+h*h*k3sx/2., 
			Sy[i-1]+h*vsy[i-1]+h*h*k3y/2., Sz[i-1]+h*vsz[i-1]+h*h*k3z/2., theta[i-1]+h*vtheta[i-1]+h*h*k3t/2.,vtheta[i-1]+h*k3t, phi[i-1]+h*vphi[i-1]+h*h*k3ph/2., 					vphi[i-1]+h*k3ph);
